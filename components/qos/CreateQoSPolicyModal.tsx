...
import {
  ...
  type CreateQoSPolicyInput,
  type QoSPriority,
  QoSPriorityLabels,
} from '@/types/qos';

interface CreateQoSPolicyModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: CreateQoSPolicyInput) => Promise<void>;
}

/**
 * QoS 정책 생성 모달
 */
export function CreateQoSPolicyModal({
  isOpen,
  onClose,
  onSubmit,
}: CreateQoSPolicyModalProps) {
  ...
  const {
    ...
    reset,
    watch,
  } = useForm<CreateQoSPolicyInput>({
    resolver: zodResolver(createQoSPolicySchema),
    defaultValues: {
      name: '',
      description: '',
      priority: 'medium',
  ...
  // 폼 제출 핸들러
  const onSubmitForm = async (data: CreateQoSPolicyInput) => {
    try {
      ...
      console.error('Failed to create QoS policy:', error);
      setSubmitError(
        error instanceof Error
          ? error.message
          : 'QoS 정책 생성에 실패했습니다. 다시 시도해주세요.'
      );
    } finally {
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                <p className="text-sm text-blue-800 dark:text-blue-200">
                  기본 정보를 입력한 후 저장하면, 트래픽 클래스 및 대역폭 할당 등
                  세부 설정을 추가할 수 있습니다.
                </p>
              </div>
            </div>
