...
import { UpdateDepartmentRequest, Department, DepartmentStatus } from '@/types/department';
...
interface EditDepartmentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (departmentId: string, data: UpdateDepartmentRequest) => Promise<void>;
...
export function EditDepartmentModal({
  ...
  // 폼 제출 핸들러
  const onSubmitForm = async (data: EditDepartmentFormData) => {
    try {
  ...
  // 자기 자신과 자신의 하위 부서는 상위 부서로 선택 불가
              ...
              <div>
                ...
                {department.memberCount > 0 && (
                  <p className="mt-1 text-xs text-amber-600 dark:text-amber-400">
                    이 부서에는 {department.memberCount}명의 인원이 소속되어 있습니다.
                  </p>
                )}
              </div>
            </div>
